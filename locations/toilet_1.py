...
# Глобальное хранилище туалетной бумаги
TOILET_PAPER_STOCK = {
...
def update_toilet_paper(toilet_id):
    ...
    # Проверяем, прошёл ли час с последнего обновления
    if now - stock["last_update"] >= timedelta(hours=1):
        # Добавляем случайное количество от 1 до 10, максимум 100
...
def user_enters_location(bot, user, location, all_users):
    paper_amount = get_toilet_paper_amount('toilet_1')

    keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
    keyboard.add(types.KeyboardButton(text="Взять туалетную бумагу"))

    # Проверяем, есть ли другие игроки для кидания бумагой
    ...
    bot.send_message(user['id'],
                     f'Вы в туалете 1 этажа. Туалетной бумаги: {paper_amount} рулонов.',
...
def user_message(bot, message, user, location, all_users):
    if message == 'Бросить туалетную бумагу':
        ...
        if paper_amount > 0:
            ...
            # Берем бумагу
            TOILET_PAPER_STOCK['toilet_1']["amount"] -= 1
            if 'toilet_paper' not in user['inventory']:
                user['inventory'].append('toilet_paper')

            bot.send_message(user['id'],
                             f'Вы взяли рулон туалетной бумаги. Осталось: {TOILET_PAPER_STOCK["toilet_1"]["amount"]} рулонов.\n'
        ...
        if location_name == 'холл 1 этажа':
            ...
            bot.send_message(user['id'], 'Отсюда можно выйти только в холл 1 этажа')
